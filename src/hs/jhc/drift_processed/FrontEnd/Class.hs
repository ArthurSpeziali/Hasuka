...
{-# LANGUAGE NoMonoLocalBinds, NamedFieldPuns, RecordWildCards, UndecidableInstances #-}
...
instance (DocLike a, Semigroup a, PPrint a (Qual Pred)) => PPrint a Inst where
...
appendCH :: ClassHierarchy -> ClassHierarchy -> ClassHierarchy
appendCH (CH a b) (CH c d) = CH (Map.union a c) (Map.unionWith Data.List.union b d)
...
methodToTopDecls :: (Monad m, MonadFail m)
    => KindEnv         -- ^ the kindenv

...
import qualified Data.List as List hiding(union,delete)
import Data.Typeable

import qualified Debug.Trace as DT

import DataConstructors(DataTable, DataTableMonad(..), onlyChild)
import Doc.DocLike(DocLike,tshow)
import Doc.PPrint(PPrint(..), pprint)
import E.E
import E.Program(programDs, programSetDs', progDataTable)
import GenUtil(fsts, snds)
import Info.Types(HasProperties(..), prop_PLACEHOLDER)
import Name.Id(IdMap, idMapToList, isEmptyId, emptyId)
import Util.HasSize(isEmpty)
import Util.SetLike(mlookup, minsert, msingleton, delete, union, fromList, keys)
...
analyze ec@ECase { eCaseBind = b, eCaseAlts = [Alt lc@LitCons { litName = h, litArgs = ts } alt], eCaseDefault = Nothing } s = do
    ...
    case onlyChild dataTable h of
        True -> do  -- product type
            ...
            (e',enve :=> sig) <- analyze (eCaseScrutinee ec) (sp [ lenv (tvrIdent t) enva | t <- ts])
            if null sig
              then do let nenv = enve `glb` foldr denvDelete enva (b:ts)
                      return (caseUpdate $ ec { eCaseScrutinee = e', eCaseAlts = [Alt lc alt'] }, nenv :=> siga)
              else trace ("Boxy WorkWrap Fallback: non-empty demand sig (len: " ++ show (length sig) ++ ")") $
                   analyzeCase ec s
        _ -> analyzeCase ec s

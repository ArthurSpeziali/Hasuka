...
{-# LANGUAGE ParallelListComp, TypeFamilies, UndecidableInstances #-}
module E.SSimplify(
    Occurance(..),
    cacheSimpOpts,
    simplifyE,
...
import qualified Data.List as List hiding(delete,union,insert)
...
appendForced :: Forced -> Forced -> Forced
appendForced NotForced x = x
appendForced x NotForced = x
appendForced _ ForceNoinline = ForceNoinline
appendForced ForceNoinline _ = ForceNoinline
...
-- Helper to resolve getType ambiguity for E and TVr
eGetType :: E -> E
eGetType = getType
tvrGetType :: TVr -> E
tvrGetType = getType
...
simplifyDs :: forall m . (MonadStats m, MonadFail m) => Program -> SimplifyOpts -> [Comb] -> m [Comb]
    ...
    f cont ELetRec { eDefs = ds@(_:_), eBody =  e } = do
        ...
        res <- case ds' of
            ...
            [(t,ec@ECase { eCaseScrutinee = sc@(EPrim p _ _), eCaseAlts = [], eCaseDefault = Just def })] | primEagerSafe p && not (getProperty prop_CYCLIC t) -> do
                ...
                return $ caseUpdate ec { eCaseDefault = Just $ ELetRec [(t,def)] e', eCaseType = eGetType e' }
            [(t,ec@ECase { eCaseScrutinee = sc@(EPrim p _ _), eCaseAlts = [Alt c def], eCaseDefault = Nothing })] | primEagerSafe p && not (getProperty prop_CYCLIC t) -> do
                ...
                return $ caseUpdate ec { eCaseAlts = [Alt c (ELetRec [(t,def)] e')], eCaseType = eGetType e' }
            _ -> do
        ...
        let
            ...
            doCase ic@ECase { eCaseType = it, eCaseScrutinee = e, eCaseBind =  b, eCaseAlts =  as, eCaseDefault =  d } t b' as' d' | not (isUnboxedTuple it) = do
                ...
                let fbody = ELam cvar rcc
                    zvar = setProperties [prop_JOINPOINT,prop_ONESHOT] $ tVr n2 (EPi tvr { tvrType = it } (eGetType rcc))
                nic <- flip caseBodiesMapM ic $ \body -> return $ eLet cvar body (eAp (EVar zvar) (EVar cvar))
                done cont $ eLet zvar fbody nic { eCaseType = eGetType rcc }
...
-}
            ...
            doCase e _ b [] (Just d) | not (isLifted e || isUnboxed (eGetType e)) = do
            ...
            doCase e _ b [] (Just d) | isUnboxed (eGetType e), isAtomic e = do
    ...
    doDs ds = do
        ...
        let z :: (InTVr,InE) -> SM (Id,UseInfo,OutTVr,InE)
            z (t,EVar t') | t == t' = do    -- look for simple loops and replace them with errors.
                ...
                return (tvrIdent t,noUseInfo,t'',EError "<<loop>>" (tvrGetType t))
            z (t,e) = do
                t' <- nname t
...
appendEnv :: Env -> Env -> Env
appendEnv (Env aa ab ac ad ae) (Env aa' ab' ac' ad' ae') = Env (mappend aa aa')(mappend ab ab')(mappend ac ac')(mappend ad ad')(mappend ae ae')

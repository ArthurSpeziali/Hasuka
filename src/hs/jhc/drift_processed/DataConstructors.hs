...
{-# LANGUAGE OverloadedStrings, RecordWildCards, ViewPatterns, ParallelListComp #-}
...
getConstructor :: (Monad m, MonadFail m) => Name -> DataTable -> m Constructor
...
getProduct :: (Monad m, MonadFail m) => DataTable -> E -> m Constructor
...
typesCompatable :: forall m . (Monad m, MonadFail m) => E -> E -> m ()
...
extractPrimitive :: (Monad m, MonadFail m) => DataTable -> E -> m (E,(ExtType,E))
...
extractIO :: (Monad m, MonadFail m) => E -> m E
...
lookupExtTypeInfo :: (Monad m, MonadFail m) => DataTable -> E -> m ExtTypeInfo
lookupExtTypeInfo dataTable oe = f Set.empty oe where
    f :: (Monad m, MonadFail m) => Set.Set Name -> E -> m ExtTypeInfo
...
expandAlias :: (Monad m, MonadFail m) => E -> m E
...
pprintTypeOfCons :: (Monad m, MonadFail m, DocLike a) => DataTable -> Name -> m a

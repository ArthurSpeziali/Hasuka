{-# LANGUAGE OverloadedStrings, ViewPatterns, NondecreasingIndentation, ParallelListComp #-}
...
getName' :: (Show a, Monad m, MonadFail m) => DataTable -> Lit a E -> m Atom
...
--                    return $ Return (Const (NodeC pt as))
    ...
    constant :: (Monad m, MonadFail m) =>  E -> m Val
    ...
    con :: (Monad m, MonadFail m) => E -> m [Val]
...
literal :: (Monad m, MonadFail m) =>  E -> m [Val]

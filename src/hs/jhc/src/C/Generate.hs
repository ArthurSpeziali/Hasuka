...
import Control.Monad.RWS(RWS,MonadState(..),MonadWriter(..),MonadReader(..),runRWS,asks,MonadFix(..),MonadFail(..))
...
instance DL.DocLike a => Semigroup (G a) where
    a <> b = do x <- a; y <- b; return (DL.hcat [x, y])
...
appendSt :: Statement -> Statement -> Statement
appendSt (St as) (St bs) = St $ pairOpt stmtPairOpt as bs
...
instance {-# OVERLAPPING #-} ToStatement Expression where
...
instance {-# OVERLAPPABLE #-} ToExpression a => ToStatement a where

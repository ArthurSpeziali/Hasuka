...
strong :: Fail.MonadFail m => [(TVr,E)] -> E -> m E
strong dsMap' term = trace ("strong: " ++ show term) $ eval' dsMap term [] where
    ...
    eval' :: Fail.MonadFail m => Map.Map TVr E -> E -> [E] -> m E
    ...
    eval' _ds (EVar TVr { tvrIdent = eid }) _stack | eid == emptyId = Fail.fail "empty ident in term"
    ...
    eval' ds e stack= Fail.fail . render $ text "Cannot strong:"
                                      <$> pprint e
                                      <$> text "With stack:"
                                      <$> pprint stack

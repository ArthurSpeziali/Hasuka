...
transformProgram tp prog = liftIO $ do
    ...
    when (dodump && dump FD.CoreSteps && (not $ Stats.null_stat estat)) $ Stats.printLStat (optStatLevel options) name estat
    when verbose $ do
    ...
    if doIterate iterate (not $ Stats.null_stat estat) then transformProgram tp { transformIterate = iterateStep iterate } prog' { progStats = istat `mappend` estat } else

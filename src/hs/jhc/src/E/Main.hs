...
processDecls cho ho' tiData = withStackStatus "processDecls" $  do
    ...
    let fint mprog = do
        ...
        mprog <- if fopts FO.TypeAnalysis
                    then transformProgram tparms { transformCategory = "typeAnalyze", transformPass = "PreInit"
        ...
        mprog <- if fopts FO.TypeAnalysis
                    then transformProgram tparms { transformCategory = "typeAnalyze", transformOperation = typeAnalyze True } mprog
                    else return mprog

        mprog <- transformProgram tparms { transformCategory = "FloatInward", transformOperation = programFloatInward } mprog
        mprog <- maybeDemandAnalyze mprog
        lintCheckProgram onerrNone mprog
...
transTypeAnalyze = transformParms { transformCategory = "typeAnalyze"
                                  , transformOperation = if fopts FO.TypeAnalysis then typeAnalyze True else return
                                  }

maybeDemandAnalyze prog
    | fopts FO.GlobalOptimize = Demand.analyzeProgram prog

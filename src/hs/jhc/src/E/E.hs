...
tvrName :: Fail.MonadFail m => TVr  -> m Name
tvrName (TVr {tvrIdent =  n }) | Just a <- fromId n = return a
tvrName tvr = Fail.fail $ "TVr is not Name: " ++ show tvr
...
fromConjured :: Fail.MonadFail m => Module -> Name -> m E
...
eToList :: Fail.MonadFail m => E -> m  [E]
...
eToList _ = Fail.fail "eToList: not list"

toString :: Fail.MonadFail m => E -> m String
toString (ELit LitCons { litName = n, litArgs = [], litType = t }) = if dc_EmptyList == n && t == tString then return "" else Fail.fail "not a string"
toString x = eToList x >>= mapM fromChar where
    ...
    fromChar _ = Fail.fail "fromChar: not char"
...
fromUnboxedTuple :: Fail.MonadFail m => E -> m [E]
fromUnboxedTuple (ELit LitCons { litName = n, litArgs = as }) | Just _ <- fromUnboxedNameTuple n = return as
fromUnboxedTuple _ = Fail.fail "fromUnboxedTuple: not a tuple"

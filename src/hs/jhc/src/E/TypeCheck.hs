{-# LANGUAGE NondecreasingIndentation, ParallelListComp, TypeFamilies #-}
module E.TypeCheck(
    canBeBox,
    eAp,
    inferType,
...
instance {-# OVERLAPPING #-} CanType E where
    type TypeOf E = E
    ...
    getType e@(EPi TVr { tvrType = a } b)
        ...
        | otherwise = maybe (error $ "E.TypeCheck.getType: " ++ show (e,getType a,getType b)) ESort $ do
            ...
            s2 <- case getType b of
                    ESort s                                  -> Just s
                    ELit LitCons { litType = ESort s }       -> Just s
                    _                                        -> Nothing
    ...
    getType e@(EAp a b) = ans where
        ans = if isUnknown typa then Unknown else if a == tBox || typa == tBox then tBox else (case a of
            -- JHC can encounter unapplied alias constructors late in the kernel
            -- pipeline; fall back to ordinary type application instead of aborting.
            (ELit LitCons {}) -> eAp typa b
            _ -> eAp typa b)
        typa = getType a
...
instance {-# OVERLAPPING #-} CanType TVr where
    type TypeOf TVr = E
    getType = tvrType
...
inferType :: (ContextMonad m, ContextOf m ~ String, Fail.MonadFail m) => DataTable -> [(TVr,E)] -> E -> m E
...
class CanTypeCheck a where
    typecheck :: (Monad m, Fail.MonadFail m) => DataTable -> a -> m E
...
typeInfer'' :: (Fail.MonadFail m, ContextMonad m, ContextOf m ~ String) => DataTable -> [(TVr,E)] -> E -> m E

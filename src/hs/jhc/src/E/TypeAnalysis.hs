...
{-# LANGUAGE NondecreasingIndentation, ParallelListComp #-}
...
typeAnalyze doSpecialize prog = do
    ...
    calcFixpoint "TypeAnalyzeMethods" fixer
...
toLit (EPi TVr { tvrType = a } b) = Just (tc_Arrow,[a,b])
toLit (ELit LitCons { litName = n, litArgs = ts }) = Just (n,ts)
toLit _ = Nothing
...
typConstant :: (Monad m, MonadFail m) => E -> m Typ
...
getTyp :: (Monad m, MonadFail m) => E -> DataTable -> Typ -> m E
...
eToPatM :: (Monad m, MonadFail m) => (E -> m TVr) -> E -> m (Lit TVr E)
...
expandPlaceholder :: (Monad m, MonadFail m) => Comb -> m Comb

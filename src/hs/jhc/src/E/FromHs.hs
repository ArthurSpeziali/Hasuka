{-# LANGUAGE NondecreasingIndentation, ParallelListComp, ViewPatterns #-}
module E.FromHs(
    convertDecls,
    convertRules,
    createInstanceRules,
...
getMainFunction :: (Monad m, MonadFail m) => DataTable -> Name -> (Map.Map Name (TVr,E)) -> m (TVr,E)
...
fromTuple_ :: (Monad m, MonadFail m) => E -> m [E]
...
actuallySpecializeE :: (Monad m, MonadFail m)
...
specializeE :: (Monad m, MonadFail m)
    => E   -- ^ the general type
    -> E   -- ^ the specific type
...
procAllSpecs :: (Monad m, MonadFail m) => DataTable -> [Type.Rule] -> [(TVr,E)] -> m ([(TVr,E)],Rules)
...
makeSpec :: (Monad m, MonadFail m) => DataTable -> (TVr,E) -> T.Rule -> m ((TVr,E),Rule)

...
appendCollectedHo :: CollectedHo -> CollectedHo -> CollectedHo
appendCollectedHo a b = updateChoHo CollectedHo {
...
mergeChoCombinators x y = unionWith f x y where
    f c1 c2 = combRules_s (combRules c1 `List.union` combRules c2) . combHead_s (merge (combHead c1) (combHead c2)) $ c1

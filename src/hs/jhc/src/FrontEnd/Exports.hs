{-# LANGUAGE RecursiveDo,NoMonoLocalBinds,RecordWildCards,ParallelListComp #-}
...
determineExports' owns doneMods todoMods = mdo
    ...
    let lf m = maybe (Fail.fail $ "determineExports'.lf: " ++ show m) return $  Map.lookup m  $ dmodMap `mappend` Map.fromList [ (modInfoName x,fromList [(toUnqualified x,x) | x <- modInfoExport x]) |  x  <- xs]
    let g  (mi,ne) = do
            ne' <- ce mi ne
    ...
    le m = runIdentity $ maybe (Fail.fail $ "determineExports'.le: " ++ show m) return $ Map.lookup m modMap
    ...
    getExports :: (Monad m, Fail.MonadFail m) => ModInfo -> (Module -> m (Rel Name Name)) -> m (Rel Name Name)
    ...
    getImports :: (Monad m, Fail.MonadFail m) => ModInfo -> (Module -> m (Rel Name Name)) -> m (Rel Name Name)

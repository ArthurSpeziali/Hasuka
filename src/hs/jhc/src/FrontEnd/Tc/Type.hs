...
extractTyVar ::  Fail.MonadFail m => Type -> m Tyvar
extractTyVar (TVar tv) = return tv
extractTyVar t = Fail.fail $ "not a Var:" ++ show t

extractMetaVar :: Fail.MonadFail m => Type -> m MetaVar
extractMetaVar (TMetaVar t)  = return t
extractMetaVar t = Fail.fail $ "not a metaTyVar:" ++ show t

extractBox :: Fail.MonadFail m => Type -> m MetaVar
extractBox (TMetaVar mv) | metaType mv > Tau  = return mv
extractBox t = Fail.fail $ "not a metaTyVar:" ++ show t
...
instance FreeVars Type [Tyvar] where
    ...
    freeVars t = foldr List.union [] $ tickleCollect ((:[]) . (freeVars :: Type -> [Tyvar])) t

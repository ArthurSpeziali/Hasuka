{-# LANGUAGE NondecreasingIndentation, ViewPatterns, RecordWildCards, ParallelListComp #-}
...
tiExpr expr@(HsLambda sloc ps e) typ = do
    ...
    let lam (p:ps) e (TMetaVar mv) rs = do -- ABS2
        ...
        lam _ _ t _ = do
            t <- flattenType t
            Fail.fail $ "expected a -> b, found: " ++ prettyPrintType t
...
tiExpr e typ = Fail.fail $ "tiExpr: not implemented for: " ++ show (e,typ)
...
tiPat (HsPNeg pat) typ = Fail.fail $ "non-literal negative patterns are not allowed"
...
tiPat (HsPApp conName pats) typ = do
    ...
    let f (p:pats) (a `TArrow` rs) (ps,env) = do
        ...
        f (p:pats) rs _ = do
            Fail.fail $ "constructor applied to too many arguments:" <+> show p <+> prettyPrintType rs
        f [] (_ `TArrow` _) _ = do
            Fail.fail "constructor not applied to enough arguments"
        f [] rs (ps,env) = do
            rs `subsumes` typ
            unBox typ

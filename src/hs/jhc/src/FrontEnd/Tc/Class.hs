...
splitPreds :: (Monad m, MonadFail m)
           => ClassHierarchy
           -> Set.Set MetaVar
           -> Preds
           -> m (Preds, Preds)
...
toHnfs      :: (Monad m, MonadFail m) => ClassHierarchy -> [Pred] -> m [Pred]
toHnfs h ps =  mapM (toHnf h) ps >>= return . concat

toHnf :: (Monad m, MonadFail m) => ClassHierarchy -> Pred -> m [Pred]
toHnf h p
    | inHnf p = return [p]
...
reducePred :: (Monad m, MonadFail m) => ClassHierarchy -> Pred -> m [Pred]
...
byInst             :: (Monad m, MonadFail m) => Pred -> Inst -> m [Pred]
byInst p Inst { instHead = ps :=> h } = do
    u <- matchPred h p
    return (map (inst mempty (Map.fromList [ (tyvarName mv,t) | (mv,t) <- u ])) ps)

matchPred :: (Monad m, MonadFail m) => Pred -> Pred -> m [(Tyvar,Type)]
...
match :: (Monad m, MonadFail m) => Type -> Type -> m [(Tyvar,Type)]

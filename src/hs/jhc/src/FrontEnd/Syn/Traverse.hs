...
instance TraverseHsOps HsExp where
    ...
    traverseHsOps hops@HsOps { .. } e = g e where
        v x = applyHsOps hops x
        g e = withSrcLoc (srcLoc e) $ f e
        f (HsCase e as)                      = HsCase <$> v e <*> v as
        f (HsDo hsStmts)                     = HsDo <$> v hsStmts
        f (HsExpTypeSig srcLoc e hsQualType) = HsExpTypeSig srcLoc <$> v e <*> v hsQualType
        f (HsLambda srcLoc hsPats e)         = HsLambda srcLoc <$> v hsPats <*> v e
        f (HsLet hsDecls e)                  = HsLet <$> v hsDecls <*> v e
        f (HsListComp c)                     = HsListComp <$> v c
        f (HsRecConstr n fus)                = HsRecConstr n <$> v fus
        f (HsRecUpdate e fus)                = HsRecUpdate <$> v e <*> v fus
        -- only exp
        f e@HsCon {}                  = pure e
        f e@HsError {}                = pure e
        f e@HsLit {}                  = pure e
        f e@HsVar {}                  = pure e
        f (HsApp a1 a2)               = HsApp <$> v a1 <*> v a2
        f (HsAsPat hsName e)          = HsAsPat hsName <$> v e
        f (HsBackTick e)              = HsBackTick <$> v e
        f (HsBangPat e)               = HsBangPat <$> v e
        f (HsEnumFrom e)              = HsEnumFrom <$> v e
        f (HsEnumFromThen e1 e2)      = HsEnumFromThen <$> v e1 <*> v e2
        f (HsEnumFromThenTo a1 a2 a3) = HsEnumFromThenTo <$> v a1 <*> v a2 <*> v a3
        f (HsEnumFromTo e1 e2)        = HsEnumFromTo <$> v e1 <*> v e2
        f (HsIf e1 e2 e3)             = liftA3 HsIf (v e1) (v e2) (v e3)
        f (HsInfixApp a1 a2 a3)       = HsInfixApp <$> v a1 <*> v a2 <*> v a3
        f (HsIrrPat hsExp)            = HsIrrPat <$> v hsExp
        f (HsLeftSection e1 e2)       = HsLeftSection <$> v e1 <*> v e2
        f (HsList hsExps)             = HsList <$> T.traverse v hsExps
        f (HsLocatedExp le)           = HsLocatedExp <$> v le
        f (HsNegApp a1)               = HsNegApp <$> v a1
        f (HsParen e)                 = HsParen <$> v e
        f (HsRightSection e1 e2)      = HsRightSection <$> v e1 <*> v e2
        f (HsTuple es)                = HsTuple <$> T.traverse v es
        f (HsUnboxedTuple es)         = HsUnboxedTuple <$> T.traverse v es
        f (HsWildCard x)              = pure (HsWildCard x)
        f (HsWords ws)                = HsWords <$> T.traverse v ws
...
instance {-# OVERLAPPABLE #-} (TraverseHsOps a,T.Traversable f) => TraverseHsOps (f a) where
    traverseHsOps hops xs = T.traverse (applyHsOps hops) xs

maybeGetDeclName :: (Monad m, MonadFail m) => HsDecl -> m Name

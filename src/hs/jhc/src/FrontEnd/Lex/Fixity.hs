...
newtype SShunt a = SShunt { runSShunt :: a }
instance Functor SShunt where fmap f (SShunt a) = SShunt (f a)
instance Applicative SShunt where pure = SShunt; SShunt f <*> SShunt a = SShunt (f a)
instance Monad SShunt where return = pure; SShunt a >>= f = f a
instance MonadFail SShunt where fail = error

simpleShunt getPrec e oes combine = runSShunt $ do
...
shuntSpec :: (MonadFail m, Show t) => ShuntSpec m t e
...
shunt :: (Show t, Show e,MonadFail m)
...
treeShuntSpec :: (MonadFail m, Show t)

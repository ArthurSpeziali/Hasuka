...
doForeign :: (Monad m, Fail.MonadFail m) => SrcLoc -> [Name] -> Maybe (String,Name) -> HsQualType -> m HsDecl
...
parseImport :: (Monad m, Fail.MonadFail m) => CallConv -> Maybe String -> Name -> m FfiType
...
parseIS :: (Monad m, Fail.MonadFail m) => CallConv -> [String] -> m FfiType

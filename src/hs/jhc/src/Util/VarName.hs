...
newName :: (Ord ni, Ord nc,MonadFail m) => [no] -> nc -> ni -> VarNameT nc ni no m no
...
maybeLookupName :: (Ord ni, MonadFail m,Show ni) => ni -> VarNameT nc ni no m (Maybe no)
...
newLookupName :: (Ord ni, Ord nc,MonadFail m) => [no] -> nc -> ni -> VarNameT nc ni no m no

...
lbsCFF :: Fail.MonadFail m => LBS.ByteString -> m (FileType,[(ChunkType,LBS.ByteString)])
lbsCFF bs' = ans bs' where
    ans bs' = do
        let checkByte n b = do
                unless ((bs `LBS.index` n) == b) $ Fail.fail "bsCFF: invalid chunked file"
            bs = LBS.take 8 bs'
        when (LBS.length bs < 8) $ Fail.fail "bsCFF: chunked file is too short"
        checkByte 0 0x89
        checkByte 4 0x0d
...
bsCFF :: Fail.MonadFail m => BS.ByteString -> m (FileType,[(ChunkType,BS.ByteString)])
bsCFF bs = ans bs where
    ans bs = do
        let checkByte n b = do
                unless ((bs `BS.index` n) == b) $ Fail.fail "bsCFF: invalid chunked file"
        when (BS.length bs < 8) $ Fail.fail "bsCFF: chunked file is too short"
        checkByte 0 0x89
        checkByte 4 0x0d
...
readCFFHeader h = do
    let checkByte b = do
            z <- getByte h
            unless (z == b) $ Fail.fail "readCFFInfo: invalid chunked file"
    checkByte 0x89
    b1 <- getByte h
    b2 <- getByte h
    b3 <- getByte h
...
readChunk h eft ect = do
    cffType <- readCFFHeader h
    when (cffType /= eft) $ Fail.fail "readChunk: CFF file of incorrect type"
    let readChunk = do
            b <- hIsEOF h
            if b then Fail.fail "readChunk: specified chunk was not found" else do
                len <- readWord32 h
                ct <- readChunkType h

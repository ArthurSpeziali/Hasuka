...
instance Semigroup Stat where
    Stat a <> Stat b = Stat (a `mappend` b)
...
newtype StatT m a = StatT (WriterT Stat m a)
    deriving(MonadIO, Functor, Applicative, MonadFix, MonadTrans, Monad)
...
instance MonadFail m => MonadFail (StatT m) where
    fail = lift . fail
...
instance {-# OVERLAPPABLE #-} (Monad m, Monad (t m), MonadTrans t, MonadStats m) => MonadStats (t m) where
...
isEmpty (Stats r) = null_stat `liftM` readIORef r
